...
data class ArtSet(
    ...
    val availableBackgroundUrls: List<String> = emptyList() // Reference: Stores remote URLs for UI preview
) {
...
class CoverArtFetcher(private val context: Context) {
    ...
    /** Finds a single exact match for a specific art type (Restored Function) */
    ...
    /** Reference: Finds all matching paths for backgrounds without downloading them. */
    private fun findAllExactPaths(gameId: String): List<String> {
        val idVariations = getGameIdVariations(gameId)
        val suffixes = listOf("_BG", "_BG_00", "_BG_01", "_BG_02", "_BG_03", "_BG.1", "_BG.2")
        val extensions = listOf(".PNG", ".JPG", ".JPEG", ".BMP")
        ...
        return pathIndex.filter { path ->
            val filename = path.substringAfterLast('/').uppercase()
            targetFilenames.contains(filename)
        }
    }

    /** Reference: Downloads ONLY the specifically chosen background when tapped by the user. */
    suspend fun downloadSpecificBackground(gameId: String, url: String): String? = withContext(Dispatchers.IO) {
        val hash = kotlin.math.abs(url.hashCode())
        val cached = File(artDir, "${gameId}_BG_$hash.png")
        if (cached.exists()) return@withContext cached.absolutePath
        
        val bytes = downloadBytes(url) ?: return@withContext null
        normalizeAndSave(bytes, cached, resizeForCover = false)
    }
    ...
    suspend fun fetchAllArt(
        ...
        onProgress("Backgrounds", "$gameId${ArtType.BACKGROUND.oplSuffix} (Searching variants...)", 4, 4)
        
        // Reference: Fetch the default one so a local file is ready immediately
        val background = fetchArt(gameId, ArtType.BACKGROUND)
        
        // Reference: Grab the raw URLs for the visual preview list
        val bgUrls = findAllExactPaths(gameId).map { (ART_DB_RAW_BASE + it).replace(" ", "%20") }
        ...
        ArtSet(
            ...
            availableBackgroundUrls = bgUrls
        )
    }
    ...
    suspend fun fetchArt(gameId: String, type: ArtType): String? = withContext(Dispatchers.IO) {
        ...
        for (consoleFolder in listOf("PS2", "PSX")) {
            for (folderId in idVariations) {
                for (fileId in idVariations) {
                    for (suffix in suffixes) {
                        for (ext in exts) {
                            val directUrl = "${ART_DB_RAW_BASE}$consoleFolder/${folderId}/${fileId}$suffix.$ext"
