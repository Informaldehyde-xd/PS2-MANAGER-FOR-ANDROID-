...
import com.ps2manager.app.util.DiscConverter
import com.ps2manager.app.util.GameIdUtil
import com.ps2manager.app.util.IsoSystemCnfReader
import com.ps2manager.app.util.RawPathResolver
...
class GameRepository(private val context: Context) {
    ...
    // ul.cfg is read, modified, and rewritten as a whole file by several functions below
    // (renameUlGame, convertIsoToUl, convertUlToIso, regenerateUlConfig). If two of those
    // ever ran at the same time — e.g. a batch operation racing with something else — each
    // would read the same "before" copy and the last write to land would silently overwrite
    // everyone else's changes. This mutex makes sure only one read-modify-write of ul.cfg
    // happens at a time, no matter what calls it.
    private val ulConfigMutex = Mutex()
    ...
    suspend fun renameFile(
        ...
        withContext(Dispatchers.IO) {
            try {
                ...
                // Force rename: same trick every real file manager uses — resolve the SAF
                // document straight to its real path on disk and rename it directly. This is
                // instant and atomic (no copying gigabytes of ISO/ZSO data around), unlike the
                // SAF DocumentsProvider rename call (which can be slow or unsupported on some
                // providers) or the copy+delete fallback below.
                val rawFile = RawPathResolver.resolve(doc.uri)
                if (rawFile != null && rawFile.exists()) {
                    val newRawFile = File(rawFile.parentFile, newName)
                    if (newRawFile.absolutePath != rawFile.absolutePath) {
                        newRawFile.delete() // force: clobber any stale file already at the destination
                    }
                    val forced = try {
    ...
    suspend fun convertIsoToZso(
        ...
        try {
            ...
            DiscConverter.isoToZso(context, sourceDoc.uri, destFile.uri, onProgress = onProgress)
    ...
    suspend fun convertZsoToIso(
        ...
        try {
            ...
            DiscConverter.zsoToIso(context, sourceDoc.uri, destFile.uri, onProgress = onProgress)
