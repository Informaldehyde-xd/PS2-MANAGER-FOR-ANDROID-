 ...
 *
 * Also transparently supports .zso (compressed ISO) images: the same PVD/SYSTEM.CNF
 * walk is performed, just reading each sector through [ZisoConverter.readLogicalRange]
 * instead of directly off the raw file.
 */
object IsoSystemCnfReader {

    /** Returns the normalized Game ID found inside the disc image's SYSTEM.CNF, or null. */
        ...
        return try {
            context.contentResolver.openFileDescriptor(isoUri, "r")?.use { pfd ->
                ParcelFileDescriptor.AutoCloseInputStream(pfd).use { stream ->
                    ...
                    val zisoHeader = ZisoConverter.readHeader(channel)
                    if (zisoHeader != null) {
                        readGameIdFromZiso(channel, zisoHeader)
                    } else {
    ...
    /** Same PVD -> root dir -> SYSTEM.CNF walk as [readGameIdFromChannel], but through a .zso image. */
    private fun readGameIdFromZiso(channel: FileChannel, header: ZisoFormat.Header): String? {
        val pvd = ZisoConverter.readLogicalRange(channel, header, 16 * SECTOR_SIZE, SECTOR_SIZE.toInt()) ?: return null
        if (pvd[1].toInt() != 'C'.code || pvd[2].toInt() != 'D'.code) {
            // Not a standard ISO9660 image at this sector — fall back to scanning the
            // first few MB of decompressed data directly for the BOOT2 string.
            return readGameIdByBruteForceZiso(channel, header)
        }
        ...
        val rootDirBytes = ZisoConverter.readLogicalRange(
            channel, header, rootExtentLba * SECTOR_SIZE, rootDataLength.toInt()
        ) ?: return null
        ...
        val fileBytes = ZisoConverter.readLogicalRange(
            channel, header, fileExtentLba * SECTOR_SIZE, fileDataLength.toInt()
        ) ?: return null
    ...
    private fun readGameIdByBruteForceZiso(channel: FileChannel, header: ZisoFormat.Header): String? {
        val scanSize = minOf(4 * 1024 * 1024L, header.totalBytes).toInt() // first 4MB is always enough on a real PS2 disc
        if (scanSize <= 0) return null
        val bytes = ZisoConverter.readLogicalRange(channel, header, 0L, scanSize) ?: return null
